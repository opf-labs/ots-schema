...
public class ObjectIdentifier {
	...
	public ObjectIdentifier(XMLStreamReader reader) throws XMLStreamException {
		parse(reader);
	}
	...
	public void output(XMLStreamWriter writer) throws XMLStreamException {
		...
		SMOutputElement objectIdentifier = frag.addElement(Premis.ns,"objectIdentifier");	
		if(xlink != null) {
			xlink.output(writer);
		}
		if(type != null)  {
			objectIdentifier.addElementWithCharacters(Premis.ns,"objectIdentifierType",type);
		}
		if(value != null)  {
			objectIdentifier.addElementWithCharacters(Premis.ns,"objectIdentifierValue",value);
		}	
		frag.closeRoot();		
	}

	public ObjectIdentifier parse(XMLStreamReader reader) throws XMLStreamException {
		reader.require(START_ELEMENT, Premis.XMLNS, "objectIdentifier");	
		...
		for (int event=reader.next(); event!=END_DOCUMENT; event=reader.next()) {
			switch(event) {
				...
				if(reader.getLocalName().equals("objectIdentifier")) {
					return this;
				}
			}
		}
